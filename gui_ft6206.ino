...
#if TOUCH_INVERT_X
  x = map(rawX, 0, TOUCH_RAW_Y_MAX, LCD_WIDTH - 1, 0);
#else
  x = map(rawX, 0, TOUCH_RAW_Y_MAX, 0, LCD_WIDTH - 1);
#endif

#if TOUCH_INVERT_Y
  y = map(rawY, 0, TOUCH_RAW_X_MAX, LCD_HEIGHT - 1, 0);
#else
  y = map(rawY, 0, TOUCH_RAW_X_MAX, 0, LCD_HEIGHT - 1);
#endif
